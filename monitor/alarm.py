...
my_token = '5081448806:AAH8p4mXI-A5uCKeTchA3l2PeaI1eAvPT1A'
...
if len(ENGINE) ==0:
  bot.sendMessage(chat_id='-1001625520916', text=str(HOSTNAME)+" ENGINE off");
  exit()
...
if int(VALIDATION) == 0:
  bot.sendMessage(chat_id='-1001625520916', text=str(HOSTNAME)+" Not Validating "+str(ERROR_TIME));
if int(ELECTION) == 0:
  bot.sendMessage(chat_id='-1001783489709', text=str(HOSTNAME)+" Not in Election "+str(ERROR_TIME));
if int(SYNC) < int(SYNCalarm):
  bot.sendMessage(chat_id='-1001625520916', text=str(HOSTNAME)+" SYNC off, SYNC:"+str(SYNC));
if int(SYNC) > int(SYNCalarm1):
  bot.sendMessage(chat_id='-1001625520916', text=str(HOSTNAME)+" SYNC off, SYNC:"+str(SYNC));
if int(DISKSPACE) > int(DISKalarm):
  bot.sendMessage(chat_id='-1001625520916', text=str(HOSTNAME)+" DISK SPACE FULL, DISK SPACE:"+str(DISKSPACE));
